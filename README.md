...
3. 서버가 PDF를 생성해서 챗봇의 공유 작업영역(`CHATBOT_OUTPUT_DIR`, 기본값 `/mnt/outputs`)에
   동일한 파일을 저장을 시도합니다.
   - **저장에 성공하면** 그 경로(`/mnt/outputs/파일명.pdf`)만 응답으로 안내합니다. 챗봇이
     해당 경로를 직접 읽어가는 구조이므로 별도의 다운로드 링크나 base64 첨부는 보내지 않습니다.
   - **저장에 실패하면**(경로가 마운트되어 있지 않은 로컬 개발 환경 등) 기존 방식으로
     폴백합니다: 15MB 이하면 파일 자체를 base64로 인코딩해서 첨부하고, 항상
     **다운로드 링크**(`/files/파일명.pdf`)도 함께 반환합니다.
4. 사용자는 챗봇 화면에서 파일을 바로 받거나(작업영역/첨부), 안내된 링크를 클릭해서 받음
...
- **챗봇 작업영역 경로 변경**: `CHATBOT_OUTPUT_DIR` 환경변수로 기본값(`/mnt/outputs`)을
  다른 경로로 바꿀 수 있습니다. 배포 환경에서 해당 경로가 챗봇과 공유되는 볼륨으로
  마운트되어 있어야 실제로 챗봇이 파일을 읽어갈 수 있고, 응답도 링크 대신 이 경로로
  안내됩니다. 로컬처럼 마운트가 안 된 환경에서는 자동으로 다운로드 링크 방식으로
  폴백합니다.
