...
3. 서버가 PDF를 생성해서:
   ...
   - 챗봇의 공유 작업영역(`CHATBOT_OUTPUT_DIR`, 기본값 `/mnt/outputs`)에도 동일한 파일을
     복사해둠 (챗봇이 해당 경로를 직접 읽어가는 구조라면 링크/첨부 없이도 파일을 사용할 수 있음).
     이 경로가 마운트되어 있지 않은 환경에서는 복사만 건너뛰고 나머지 응답은 정상 진행됩니다.
...
- **챗봇 작업영역 경로 변경**: `CHATBOT_OUTPUT_DIR` 환경변수로 기본값(`/mnt/outputs`)을
  ...
  마운트되어 있어야 실제로 챗봇이 파일을 읽어갈 수 있습니다.
