...
- 사내 전용이라도 `MCP_AUTH_TOKEN`으로 최소한의 인증을 걸어두는 걸 권장합니다.
  단, `/files/:fileName` 다운로드 라우트는 이 토큰 검사에서 **제외**되어 있습니다.
  사용자가 브라우저에서 링크를 직접 클릭하는 용도라 `Authorization` 헤더를 붙일
  방법이 없기 때문입니다. `/mcp` 도구 호출 자체는 여전히 토큰으로 보호됩니다.
- HTTPS 뒤에 배포하세요 (사내 리버스 프록시 등).
- `/files/:fileName` 라우트는 경로 탈출(`../` 등)을 막도록 `path.basename`으로
  파일명을 정리하고, 파일명에 랜덤 8자리가 붙어 추측하기 어려운 공유 링크
  수준의 보호는 됩니다. 그래도 외부에 노출되는 엔드포인트인 만큼 민감한 문서라면
  내부망에서만 접근되도록 네트워크 단에서도 막아두는 게 안전합니다.
